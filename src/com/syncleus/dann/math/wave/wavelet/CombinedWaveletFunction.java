...
public class CombinedWaveletFunction extends AbstractFunction implements Cloneable
{
    ...
    public double calculate()
    {
        ...
        //was getting concurrent mod exception here so changed it to iterative instead of for each
        for(int i = 0; i < this.waves.size(); i++)
        {
        	WaveMultidimensionalFunction currentWave = this.waves.get(i);
            for(String dimension:this.dimensions)
            {
            	if (Arrays.asList(currentWave.getDimensionNames()).contains(dimension))
