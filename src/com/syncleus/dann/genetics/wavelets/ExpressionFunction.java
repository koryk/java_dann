...
public class ExpressionFunction implements Cloneable
{
	...
	public double calculate(final Set<SignalKeyConcentration> signalConcentrations)
	{
		...
		for(ReceptorKey receptor : this.receptors)
		{
			...
			if (this.wavelet.getParameterNames().contains(String.valueOf(receptor.hashCode())))
				this.wavelet.setParameter(String.valueOf(receptor.hashCode()), concentration);
			else
				System.out.print("missed adding bind");
		}
				...
				for(WaveMultidimensionalFunction wave:this.waves)
				{
					...
					for(String dimension:dimensionNames)
					{
						...
						newWave.setDimension(dimension, wave.getDimension(dimension));
							...
							continue;//there is something wrong with initializing the waves, because it doesn't think dimensions exist sometimes.
						}
					}
    ...
    private WaveMultidimensionalFunction generateNewWave()
    {
        ...
        newWave.setDistribution(RANDOM.nextFloat() * 100 + 1);
