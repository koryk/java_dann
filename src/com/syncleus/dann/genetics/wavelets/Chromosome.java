...
{
	protected WaveletChromatid leftChromatid;
	protected WaveletChromatid rightChromatid;
	...
	public Chromosome()
	{
		...
		this.mutability = RANDOM.nextDouble()*10;
	}
	...
	private void crossover(final double deviation)
	{
		...
		int crossoverPosition;
			...
			final int rightLength = this.rightChromatid.getGenes().size() - this.rightChromatid.getCentromerePosition();

			int length = ( leftLength < rightLength ? leftLength : rightLength);
			if (length < 1)
				crossoverPosition = 0;
			else 
				crossoverPosition = (int)((length-1)*Math.abs(Mutations.getRandom().nextDouble()));
		

		//perform the crossover.
		final List<AbstractWaveletGene> leftGenes = this.leftChromatid.clone().crossover(crossoverPosition);
		final List<AbstractWaveletGene> rightGenes = this.rightChromatid.clone().crossover(crossoverPosition);

		this.leftChromatid.crossover(rightGenes, crossoverPosition-leftChromatid.getCentromerePosition());
		this.rightChromatid.crossover(leftGenes, crossoverPosition-rightChromatid.getCentromerePosition());
	}
