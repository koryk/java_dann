...
public class WaveletChromatid implements Chromatid<AbstractWaveletGene>, Cloneable
{
	...
	public void tick()
	{
		...
		{
			this.sequencedGenes.get(sequenceIndex).tick(promotions.containsKey(sequenceIndex)? promotions.get(sequenceIndex) : 0);
		}
	}
	...
	public void mutate(final Set<AbstractKey> keyPool)
	{
		...
		if(Mutations.mutationEvent(mutability))
			if (this.localSignalGenes.size() > 0)
				this.sequencedGenes.remove(this.localSignalGenes.remove(Mutations.getRandom().nextInt(this.localSignalGenes.size())));
